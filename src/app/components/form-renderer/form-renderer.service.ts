...
import {
  ...
  optionsType,
  widgetType,
} from 'src/app/constants/dynamic-form.constant';
...
export class FormRendererService {
    ...
    for (let i = 0; i < data.length; i++) {
      this.form.addControl(
        ...
        this.fb.control(
          data[i].value,
          data[i] &&
            data[i].validationType &&
            JSON.parse(data[i].validationType).required
            ? Validators.required
            : null
        )
      );
      if (data[i].optionsType === optionsType.data_set) {
        this.getDataSets(data[i].optionsDataSetId, data[i].value).subscribe(
          (response: IApiResult) => {
            if (response.success) {
      ...
      if (
        data[i].optionsType === optionsType.cascade &&
        data[i].optionsDataSetId
      ) {
        this.getDataSets(data[i].optionsDataSetId, data[i].value).subscribe(
          (response: IApiResult) => {
            if (response.success) {
              ...
              if (data[i].value) {
                let index = data
                  ...
                  .indexOf(data[i].extra.field_name);
                if (index != -1) {
                  // this.form.get(data[index].field_name).setValue(null); // clears child
                  this.getCascadeDataSets(
                    data[i].extra.optionDataSetId,
                    data[i].value,
                    data[i].value
  ...
  getDataSets(id: number, selectedValue: number): any {
    const endpoint = `${CUSTOM_FORM_URLs.loadDataSets}/${id}${
      selectedValue ? '?datasetValue=' + selectedValue : ''
    }`;
  ...
  getCascadeDataSets(
    cascadeId: any,
    cascadeTypeId: number,
    selectedValue: number
  ): any {
    return this.apiService.read(
      `${CUSTOM_FORM_URLs.loadCascadeDataSets}/${cascadeTypeId}/${cascadeId}${
        selectedValue ? '?datasetValue=' + selectedValue : ''
      }`
    );
  }
